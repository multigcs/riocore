...
typedef struct {
    ...
    hal_bit_t   *SIGOUT_BOARD0_WLED_0_GREEN;
    hal_bit_t   *SIGOUT_BOARD0_WLED_0_BLUE;
    hal_bit_t   *SIGOUT_BOARD0_WLED_0_RED;
...
void register_signals(void) {
    ...
    *data->duration = rtapi_get_time();
    if (retval = hal_pin_bit_newf(HAL_IN, &(data->SIGOUT_BOARD0_WLED_0_GREEN), comp_id, "board0.wled.0_green") != 0) error_handler(retval);
    *data->SIGOUT_BOARD0_WLED_0_GREEN = 0;
    if (retval = hal_pin_bit_newf(HAL_IN, &(data->SIGOUT_BOARD0_WLED_0_BLUE), comp_id, "board0.wled.0_blue") != 0) error_handler(retval);
    *data->SIGOUT_BOARD0_WLED_0_BLUE = 0;
    if (retval = hal_pin_bit_newf(HAL_IN, &(data->SIGOUT_BOARD0_WLED_0_RED), comp_id, "board0.wled.0_red") != 0) error_handler(retval);
    *data->SIGOUT_BOARD0_WLED_0_RED = 0;
...
void write_txbuffer(uint8_t *txBuffer) {
    ...
    txBuffer[16] |= (data->VAROUT1_WLED_0_GREEN<<7); // 32
    txBuffer[16] |= (data->VAROUT1_WLED_0_BLUE<<6); // 31
    txBuffer[16] |= (data->VAROUT1_WLED_0_RED<<5); // 30
