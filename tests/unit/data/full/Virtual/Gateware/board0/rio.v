...
module rio (
    ...
    wire [BUFFER_SIZE_RX-1:0] rx_data_board0_w5500;
    wire [BUFFER_SIZE_RX-1:0] rx_data = rx_data_board0_w5500;

    // generate sync edge (board0_w5500)
    wire INTERFACE_SYNC_BOARD0_W5500;
    reg[2:0] INTERFACE_SYNC_BOARD0_W5500_REG;
    wire INTERFACE_SYNC_BOARD0_W5500_RISINGEDGE = (INTERFACE_SYNC_BOARD0_W5500_REG[2:1]==2'b01);
    always @(posedge sysclk) begin
        INTERFACE_SYNC_BOARD0_W5500_REG <= {INTERFACE_SYNC_BOARD0_W5500_REG[1:0], INTERFACE_SYNC_BOARD0_W5500};
    end

    // check interface timeout via sync flag
    wire INTERFACE_SYNC_RISINGEDGE = INTERFACE_SYNC_BOARD0_W5500_RISINGEDGE;
    reg [TIMEOUT_BITS-1:0] timeout_counter = 0;
    reg INTERFACE_TIMEOUT = 0;
    always @(posedge sysclk) begin
        if (INTERFACE_SYNC_BOARD0_W5500_RISINGEDGE == 1) begin
            timeout_counter <= 0;
        end else begin
    ...
    w5500 #(
        ...
        .intr(1'd0),
        .rx_data(rx_data_board0_w5500),
        .tx_data(tx_data),
        .sync(INTERFACE_SYNC_BOARD0_W5500)
    );
