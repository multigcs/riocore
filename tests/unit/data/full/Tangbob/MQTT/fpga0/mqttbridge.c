...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        sprintf(tmp_str, "%f", *data->SIGIN_FPGA0_MODBUS0_TEMPERATURE);
        pubmsg.payload = tmp_str;
        pubmsg.payloadlen = (int)strlen(tmp_str);
        pubmsg.qos = 0;
        pubmsg.retained = 0;
        if ((rc = MQTTClient_publishMessage(client, "fpga0/fpga0/modbus0/temperature", &pubmsg, &token)) != MQTTCLIENT_SUCCESS) {
