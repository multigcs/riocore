...
void convert_frame_modbus0_output(data_t *data) {
    ...
    if (timeout == 0 || frame_timeout == 1 || (frame_ack == 1 && (float)(stamp_last - modbus0_last_rx) / 1000000.0 > delay)) {
        ...
        if (prio_selection == 1) {
            for (uint8_t i = 0; i < MODBUS0_MAX; i++) {
                if (prio_next < MODBUS0_MAX - 1) {
                    prio_next++;
                } else {
                    prio_next = 0;
                }
        ...
        if (prio_selected == 0) {
            ...
            modbus0_signal_active = signal_next;
            prio_selection = 1;
        } else {
            prio_selection = 0;
        }
        last_selection = modbus0_signal_active;
        ...
        if (frame_len == 0) {
            ...
            memcpy(modbus0_frame_last_tx, frame_data, 16);
            modbus0_frame_last_tx_len = frame_len;
        }
        /*******************/
