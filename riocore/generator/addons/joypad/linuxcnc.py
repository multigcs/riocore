def _gui(parent):
    ...
    output.append("")
    if joypad_btn_slow:
        parent.custom_net_add(f"input.0.{joypad_btn_slow}", f"joy_or2_sel0.in0")
    if joypad_btn_medium:
        parent.custom_net_add(f"input.0.{joypad_btn_medium}", f"joy_or2_sel1.in0")
    if joypad_btn_fast:
        parent.custom_net_add(f"input.0.{joypad_btn_fast}", f"joy_or2_sel0.in1")
        parent.custom_net_add(f"input.0.{joypad_btn_fast}", f"joy_or2_sel1.in1")

    parent.custom_net_add(f"joy_or2_sel0.out", f"joy_mux4.sel0")
    parent.custom_net_add(f"joy_or2_sel1.out", f"joy_mux4.sel1")
    parent.custom_net_add(f"joy_mux4.out", f"halui.axis.jog-speed")
    parent.custom_net_add(f"joy_mux4.out", f"halui.joint.jog-speed")
    ...
    for axis_name, joints in parent.axis_dict.items():
        ...
        if jaxis:
            ...
            parent.custom_net_add(f"halui.machine.is-on", f"mux2_{axis_lower}.sel")
            parent.custom_net_add(f"input.0.{jaxis}-position", f"mux2_{axis_lower}.in1")
            parent.custom_net_add(f"mux2_{axis_lower}.out", f"halui.axis.{axis_lower}.analog")
            for joint, joint_setup in joints.items():
                parent.custom_net_add(f"mux2_{axis_lower}.out", f"halui.joint.{joint}.analog")
    return output
...
def hal(parent):
    ...
    output.append("")
    if joypad_btn_slow:
        parent.hal_net_add(f"input.0.{joypad_btn_slow}", f"joy_or2_sel0.in0")
    if joypad_btn_medium:
        parent.hal_net_add(f"input.0.{joypad_btn_medium}", f"joy_or2_sel1.in0")
    if joypad_btn_fast:
        parent.hal_net_add(f"input.0.{joypad_btn_fast}", f"joy_or2_sel0.in1")
        parent.hal_net_add(f"input.0.{joypad_btn_fast}", f"joy_or2_sel1.in1")
    ...
    for axis_name, joints in parent.axis_dict.items():
        ...
        if jaxis:
            ...
            if reverse:
                output.append(f"setp input.0.{jaxis}-scale -127.5")
            output.append(f"addf mux2_{axis_lower} servo-thread")
            parent.hal_net_add(f"halui.machine.is-on", f"mux2_{axis_lower}.sel")
