...
from PyQt5.QtGui import QPixmap, QStandardItemModel, QPainter, QPen, QBrush, QFont, QColor
...
class MyDrawing(QWidget):
    ...
    def runTimer(self):
        HOME_SEARCH_VEL = self.parent.setup["HOME_SEARCH_VEL"]["widget"].value()
        HOME = self.parent.setup["HOME"]["widget"].value()
        MIN_LIMIT = self.parent.setup["MIN_LIMIT"]["widget"].value()
        MAX_LIMIT = self.parent.setup["MAX_LIMIT"]["widget"].value()

        self.invert = 1
        if HOME_SEARCH_VEL > 0:
            self.invert = -1

        self.parent.setup["HOME_SEARCH_VEL"]["label"].setStyleSheet("QLabel { color : black; }")
        self.parent.setup["HOME_LATCH_VEL"]["label"].setStyleSheet("QLabel { color : black; }")
        self.parent.setup["HOME_FINAL_VEL"]["label"].setStyleSheet("QLabel { color : black; }")
        speed = 0
        if f"HOME_{self.used_speed}_VEL" in self.parent.setup:
            speed = abs(self.parent.setup[f"HOME_{self.used_speed}_VEL"]["widget"].value())
            self.parent.setup[f"HOME_{self.used_speed}_VEL"]["label"].setStyleSheet("QLabel { color : blue; }")
        ...
        elif abs(self.parent.setup["HOME_LATCH_VEL"]["widget"].value()) >= abs(self.parent.setup["HOME_SEARCH_VEL"]["widget"].value()):
    ...
    def paintEvent(self, event):
        ...
        HOME_SEARCH_VEL = self.parent.setup["HOME_SEARCH_VEL"]["widget"].value()
        HOME_LATCH_VEL = self.parent.setup["HOME_LATCH_VEL"]["widget"].value()
        HOME_OFFSET = self.parent.setup["HOME_OFFSET"]["widget"].value()
        HOME = self.parent.setup["HOME"]["widget"].value()
        MIN_LIMIT = self.parent.setup["MIN_LIMIT"]["widget"].value()
        MAX_LIMIT = self.parent.setup["MAX_LIMIT"]["widget"].value()
