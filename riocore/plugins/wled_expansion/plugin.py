...
class Plugin(PluginBase):
    ...
    def gateware_defines(self, direct=False):
        defines = []
        num_leds = self.plugin_setup.get("leds", 1)
        defines.append(f"reg [{num_leds - 1}:0] {self.expansion_prefix}_GREEN;")
        defines.append(f"reg [{num_leds - 1}:0] {self.expansion_prefix}_BLUE;")
        defines.append(f"reg [{num_leds - 1}:0] {self.expansion_prefix}_RED;")
