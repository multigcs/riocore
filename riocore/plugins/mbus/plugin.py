...
class Plugin(PluginBase):
    ...
    def frameio_rx_c(self):
        debug = self.plugin_setup.get("debug", self.option_default("debug"))
        output = []
        ...
        if debug:
            ...
            output.append(f'            printf("< MODBUS_RX{self.instances_name}: ");')
            output.append("            for (int i = 0; i < frame_len; i++) {")
            output.append('                printf("%i ", frame_data[i]);')
            output.append("            }")
    ...
    def frameio_tx_c(self):
        ...
        output.append("        static int16_t signal_next = 0;")
        output.append("        static int16_t prio_next = 0;")
        ...
        if debug:
            output.append('        printf("\\n");')
            output.append(f'        printf("> MODBUS_TX({self.instances_name}): ");')
            output.append("        for (int i = 0; i < frame_len; i++) {")
            output.append('            printf("%i ", frame_data[i]);')
            output.append("        }")
            output.append('        printf("\\n");')
