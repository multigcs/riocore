...
class Plugin(PluginBase):
    ...
    def func_tx(self):
        ...
        uprefix = self.PREFIX.replace(".", "_").upper()
        num_config_registers = len(self.HYVFD_CONFIG_REGISTER)
        ...
        output.append(f"            if (*data->SIGIN_{uprefix}_RATED_MOTOR_REV > 0.0) {{")
        output.append(f"               hz_per_rpm = *data->SIGIN_{uprefix}_MAX_FREQ / *data->SIGIN_{uprefix}_RATED_MOTOR_REV;")
        output.append("            }")
        output.append(f"            float value =  fabs((*data->SIGOUT_{uprefix}_SPEED_COMMAND + freq_comp) * hz_per_rpm);")
        output.append(f"            if (value > *data->SIGIN_{uprefix}_MAX_FREQ) {{")
        output.append(f"                value = *data->SIGIN_{uprefix}_MAX_FREQ;")
        output.append("            }")
        output.append(f"            if (value < *data->SIGIN_{uprefix}_FREQ_LOWER_LIMIT) {{")
        output.append(f"                value = *data->SIGIN_{uprefix}_FREQ_LOWER_LIMIT;")
        output.append("            }")
        output.append("            uint16_t value_int = value * 100.0;")
        output.append(f"            if (value_int != {self.instances_name}_speed_last) {{")
        output.append(f"                *data->SIGIN_{uprefix}_AT_SPEED = 0;")
        output.append(f"                frame_data[0] = {address};")
        ...
        output.append(f"            if (*data->SIGOUT_{uprefix}_SPINDLE_ON == 0) {{")
        output.append("                // STOP;")
        output.append("                frame_data[3] = 0x08;")
        output.append(f"            }} else if (*data->SIGOUT_{uprefix}_SPINDLE_FORWARD == 1) {{")
        output.append("                // FWD;")
        output.append("                frame_data[3] = 0x01;")
        output.append(f"            }} else if (*data->SIGOUT_{uprefix}_SPINDLE_REVERSE == 1) {{")
        output.append("                // REV;")
    ...
    def func_rx(self):
        ...
        output.append("                }")
        output.append("            }")
        output.append(f"            *data->SIGIN_{uprefix}_HYCOMM_OK = 1;")
        ...
        for register, data in self.HYVFD_STATUS_REGISTER.items():
            ...
            output.append(f"                        *data->SIGIN_{uprefix}_{data['name'].upper()} = {self.instances_name}_status_register[n].value * {data['scale']};")
            output.append("                    }")
            vn += 1
        output.append("                    break;")
        output.append("                }")
        output.append("            }")
        output.append(f"            if (*data->SIGIN_{uprefix}_MAX_FREQ > 0.0) {{")
        output.append(f"                *data->SIGIN_{uprefix}_SPEED_FB = *data->SIGIN_{uprefix}_FRQ_GET / *data->SIGIN_{uprefix}_MAX_FREQ * *data->SIGIN_{uprefix}_RATED_MOTOR_REV * {self.HYVFD_CALC_KEYS['speed_fb']['scale']};")
        output.append("            }")
        output.append(f"            *data->SIGIN_{uprefix}_SPEED_FB_RPS = *data->SIGIN_{uprefix}_SPEED_FB / 60.0;")
        output.append(f"            if (*data->SIGOUT_{uprefix}_SPINDLE_AT_SPEED_TOLERANCE == 0.0) {{")
        output.append(f"                *data->SIGOUT_{uprefix}_SPINDLE_AT_SPEED_TOLERANCE = 5.0;")
        output.append("            }")
        output.append(f"            float tolerance =  fabs(*data->SIGOUT_{uprefix}_SPEED_COMMAND) * *data->SIGOUT_{uprefix}_SPINDLE_AT_SPEED_TOLERANCE / 100.0;")
        output.append(f"            float diff =  fabs(*data->SIGIN_{uprefix}_SPEED_FB) -  fabs(*data->SIGOUT_{uprefix}_SPEED_COMMAND);")
        output.append("            if (diff <= tolerance) {")
        output.append(f"                *data->SIGIN_{uprefix}_AT_SPEED = 1;")
        output.append("            } else {")
        output.append(f"                *data->SIGIN_{uprefix}_AT_SPEED = 0;")
        output.append("            }")
        output.append(f"            *data->SIGIN_{uprefix}_HYCOMM_OK = 1;")
        output.append(f"            if (*data->SIGIN_{uprefix}_ERROR_COUNT > 0) {{")
        output.append(f"                *data->SIGIN_{uprefix}_ERROR_COUNT -= 1;")
        output.append("            }")
        ...
        output.append("            // ERROR")
        output.append(f"            *data->SIGIN_{uprefix}_ERROR_COUNT += 1;")
        output.append(f"            *data->SIGIN_{uprefix}_HYCOMM_OK = 0;")
        output.append(f"            *data->SIGIN_{uprefix}_AT_SPEED = 0;")
        output.append("        }")
        output.append(f"        *data->SIGIN_{uprefix}_BASE_FREQ = {self.instances_name}_config_register[0].value * {self.HYVFD_CALC_KEYS['base_freq']['scale']};")
        output.append(f"        *data->SIGIN_{uprefix}_MAX_FREQ = {self.instances_name}_config_register[1].value * {self.HYVFD_CALC_KEYS['max_freq']['scale']};")
        output.append(f"        *data->SIGIN_{uprefix}_FREQ_LOWER_LIMIT = {self.instances_name}_config_register[2].value * {self.HYVFD_CALC_KEYS['freq_lower_limit']['scale']};")
        output.append(f"        *data->SIGIN_{uprefix}_RATED_MOTOR_VOLTAGE = {self.instances_name}_config_register[3].value * {self.HYVFD_CALC_KEYS['rated_motor_voltage']['scale']};")
        output.append(f"        *data->SIGIN_{uprefix}_RATED_MOTOR_CURRENT = {self.instances_name}_config_register[4].value * {self.HYVFD_CALC_KEYS['rated_motor_current']['scale']};")
        output.append(f"        *data->SIGIN_{uprefix}_RPM_AT_50HZ = {self.instances_name}_config_register[5].value * {self.HYVFD_CALC_KEYS['rpm_at_50hz']['scale']};")
        output.append(f"        *data->SIGIN_{uprefix}_RATED_MOTOR_REV = (*data->SIGIN_{uprefix}_RPM_AT_50HZ / 50.0) * *data->SIGIN_{uprefix}_MAX_FREQ;")
        output.append(f"        *data->SIGIN_{uprefix}_VFD_ERRORS = *data->SIGIN_{uprefix}_ERROR_COUNT;")
        output.append("    }")
        output.append("}")
        output.append("")
        return output
