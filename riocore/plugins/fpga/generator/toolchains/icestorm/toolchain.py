...
""",
    ...
    def generate(self, path):
        ...
        if sys.platform == "linux":
            ...
            if nextpnr is None:
                print(f"WARNING: toolchain not found installation in PATH: nextpnr (nextpnr-{family})")
        ...
        if family == "ecp5":
            ...
            pins_generator = importlib.import_module(".pins", "riocore.plugins.fpga.generator.pins.ccf")
            bitfileName = "$(PROJECT).bit"
        ...
        if flashcmd:
            ...
            if board and board.startswith("ICESugarNano"):
                ...
                makefile_data.append("load:")
                makefile_data.append(f"	 openFPGALoader -b ice40_generic {bitfileName} -f")
