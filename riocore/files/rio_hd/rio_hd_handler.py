...
from qtvcp.core import Action, Info, Path, Qhal, Status
...
""" % (
    ...
    def comp_count_changed(self, data):
        self.w.z_comp_eoffset_value.setText(format(data * 0.001, ".3f"))
    ...
    def btn_pause_spindle_clicked(self, state):
        ...
        if state:
            ...
            if STATUS.is_auto_running():
                ...
                mess = {"NAME": "MESSAGE", "ICON": "WARNING", "ID": "_wait_resume_", "MESSAGE": "CAUTION", "NONBLOCKING": "True", "MORE": info, "TYPE": "OK"}
                ACTION.CALL_DIALOG(mess)
    ...
    def btn_load_file_clicked(self):
        if self.w.btn_gcode_edit.isChecked():
            return
    ...
    def update_gcode_properties(self, props):
        ...
        property_names = {
            ...
            "g": "Total distance:",
            "run": "Run time:",
            "machine_unit_sys": "Machine Unit System:",
            "x": "X bounds:",
            "x_zero_rxy": "X @ Zero Rotation:",
            "y": "Y bounds:",
            "y_zero_rxy": "Y @ Zero Rotation:",
            "z": "Z bounds:",
            "z_zero_rxy": "Z @ Zero Rotation:",
            "a": "A bounds:",
            "b": "B bounds:",
            "c": "C bounds:",
            "toollist": "Tool Change List:",
        ...
        if props:
            for i in props:
                ...
                mess += (
                    '<span style=" font-size:18pt; font-weight:600; color:black;">%s </span>\
<span style=" font-size:18pt; font-weight:600; color:#aa0000;">%s</span>\
<br>'
                    % (property_names.get(i), props[i])
                )
    ...
    def set_style_default(self):
        self.w.statusbar.setStyleSheet("background-color: rgb(252, 252, 252);color: rgb(0,0,0)")  # default white

    def set_style_warning(self):
        self.w.statusbar.setStyleSheet("background-color: rgb(242, 246, 103);color: rgb(0,0,0)")  # yellow
        self.endcolor()

    def set_style_critical(self):
        self.w.statusbar.setStyleSheet("background-color: rgb(255, 144, 0);color: rgb(0,0,0)")  # orange
        self.endcolor()
    ...
    def adjust_stacked_widgets(self, requestedIndex):
        ...
        if mode == STATUS.AUTO:
            seq = {
                TAB_MAIN: (TAB_MAIN, PAGE_GCODE, SHOW_DRO),
            ...
            seq = {
                TAB_MAIN: (requestedIndex, PAGE_GCODE, SHOW_DRO),
