...
char *name;
RTAPI_MP_STRING(name, "instance name");
...
int rtapi_app_main(void) {
    ...
    char cmd[1024];
    sprintf(cmd, "halcompile --compile riocomp-%s.c > /dev/null", name);
	if (system(cmd) != 0) {
		printf(".failed\n");
        return -1;
	}
	// set absolut path to the rio component
    if (snprintf(so_file, sizeof(so_file), "%s/riocomp-%s.so", cwd, name) < 0) {
		printf(".failed\n");
        fputs("getcwd() error\n", stderr);
        return -1;
    }
