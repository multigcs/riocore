            ...
            for plugin_instance in self.plugin_instances:
                for gpio_pin, gpio_data in plugin_instance.PINDEFAULTS.items():
                    if "source" in gpio_data:
                        ...
                        # pins of sub plugins
                        if gpio_pin in plugin_instance.PINDEFAULTS and gpio_data["source"].startswith(f"{plugin_instance.instances_name}_"):
...
class PluginImages:
    ...
    def __init__(self):
        self.images = json.loads(open(os.path.join(riocore_path, "files", "images", "pins.json"), "r").read())

    def kicad(self, plugin, node_type=None):
        result = {}
        images = json.loads(open(os.path.join(riocore_path, "kicad-images.json"), "r").read()).get(plugin) or {}
        for name, data in images.items():
            if data["info"]["node_type"] and node_type != data["info"]["node_type"]:
                continue
            result[name] = data
        return result
